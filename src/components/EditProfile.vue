  ...
  <div id="edit-profile" class="edit-profile container ">
    <div class="card edit-card ">
      ...
      <div class="card-body">
        ...
        <button type="button" class="btn btn-secondary" @click="checkPassword()" >Change Password</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    checkPassword() {
      ...
      if(this.password == clickUser.password && this.newPassword == this.confirmPassword) {
        let newPassword = this.newPassword;
        let authUsers = JSON.parse(localStorage.getItem("authUser"));
        if(authUsers){
          authUsers.find((item)=>{
            if( item.email == clickUser.email && item.password == clickUser.password){
